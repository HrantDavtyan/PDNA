...
function(input, output,session) {
  ...
  # calculating total loss/damage per effect per community
  for (effecto in c("an_cr_entry","tree_entry","asset_entry","land_entry","livestock_entry")){
    mydb = connect_to_db()
    rs = dbSendQuery(mydb, paste0("SELECT Com_ID,SUM(damages),SUM(loss)  FROM ",effecto," GROUP BY Com_ID;"))
    rs = fetch(rs, n=-1)
    if(exists("map_database_creation")){
      map_database_creation<-rbind(map_database_creation,rs)
    }
    else{
      map_database_creation<-rs
    }
  }
  map_database<<-aggregate(map_database_creation[2:3],map_database_creation["Com_ID"],sum)
